...
{
   Item *data_pre_rehash = data;
   size_type capacity_pre_rehash = capacity;
  ...
  for(size_type i = 0; i < capacity_pre_rehash; ++i)
  {
    if(data_pre_rehash[i].word[0] != '\0')
    {
      insert(data_pre_rehash[i].word);
    }
  }
